...
from src.phase0.data.repository import get_session, init_db
...
def load_clean_to_db(clean_path: Path) -> None:
    ...
    with get_session() as session:
        for _, row in df.iterrows():
            ...
            restaurant.rating = float(row["rating"]) if row.get("rating") is not None else None
            restaurant.votes = int(row["votes"]) if row.get("votes") is not None else None
            restaurant.features = ""
