...
def infer_column_mapping(df: pd.DataFrame) -> ColumnMapping:
    """Infer dataset column names from common Zomato field patterns."""
...
def transform_raw_to_restaurants(raw_path: Path) -> Path:
    ...
    transformed["city"] = df[mapping.city].map(normalize_city)
    transformed["locality"] = (
        df[mapping.locality].map(normalize_locality) if mapping.locality else ""
    )
    ...
    transformed["votes"] = (
        df[mapping.votes].map(coerce_int) if mapping.votes else None
    )

    # basic dedupe: restaurant + city + locality
    ...
    transformed = transformed.drop_duplicates(subset=["dedupe_key"])
    transformed = transformed.drop(columns=["dedupe_key"])
...
if __name__ == "__main__":
    raw_file = RAW_DIR / "zomato_raw.parquet"
    if not raw_file.exists():
        raise SystemExit(f"Raw file not found at {raw_file}. Run ingest_hf_dataset.py first.")
    out = transform_raw_to_restaurants(raw_file)
    print(f"Wrote transformed restaurants to {out}")
